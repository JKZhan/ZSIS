...
class BBoxTestMixin(object):
    ...
    def simple_test_bboxes(self,
        ...
        if with_decoder:
            cls_score, bbox_pred, _, _, T_bg = self.bbox_head(roi_feats, bg_vector=bg_vector)
        else:
            cls_score, bbox_pred = self.bbox_head(roi_feats, bg_vector=bg_vector)
        img_shape = img_meta[0]['img_shape']
        scale_factor = img_meta[0]['scale_factor']
        det_bboxes, det_labels, det_bboxes_idxs = self.bbox_head.get_det_bboxes(
            rois,
            cls_score,
            bbox_pred,
            img_shape,
        ...
        return det_bboxes, det_labels, det_bboxes_idxs, T_bg
...
class MaskTestMixin(object):
    ...
    def simple_test_mask(self,
        ...
        if det_bboxes.shape[0] == 0:
            ...
            if self.gzsd_mode:
                seen_mask_pred, unseen_mask_pred = self.mask_head(mask_feats, bg_vector,  det_bboxes_idxs, T_bg)
            ...
            if not self.gzsd_mode:
                mask_pred = self.mask_head(mask_feats, bg_vector, det_bboxes_idxs, T_bg )
                segm_result = self.mask_head.get_seg_masks(mask_pred, _bboxes,
                                                           det_labels,
                                                           self.test_cfg.rcnn,
                                                           ori_shape, scale_factor,
