...
class BBoxSemanticHead(nn.Module):
    ...
    def __init__(self,
        ...
        if self.with_semantic:
            ...
            vec_all = vec_load
            vec_all = torch.tensor(vec_all, dtype=torch.float32)
            self.vec_all = vec_all.cuda()
            # if self.seen_class:
        ...
        self.sync_Vb = sync_Vb
        self.sync_T_bg = sync_T_bg
    ...
    def get_det_bboxes(self,
        ...
        # scores = semantic_score if semantic_score is not None else None
        ...
        if self.gzsd:
            ...
            semantic_bg = torch.mm(semantic_score, T_bg.t())
            seen_scores = torch.mm(semantic_score, self.vec)
            semantic_bg = torch.diagonal(semantic_bg, 0)
            
        
            seen_scores[:,0] = semantic_bg
            ...
            unseen_scores =torch.mm(semantic_score, self.vec_unseen)
            unseen_scores[:,0] = semantic_bg
            ...
            if cfg is None:
                ...
                seen_det_bboxes, seen_det_labels,  seen_det_bboxes_idxs = multiclass_nms(seen_bboxes, seen_scores,
                                                        # 0.2, cfg.nms,
                                                        0.05, cfg.nms,
                                                        cfg.max_per_img)
                unseen_det_bboxes, unseen_det_labels, unseen_det_bboxes_idxs = multiclass_nms(unseen_bboxes, unseen_scores,
                                                                  0.05, cfg.nms,
                                                                  cfg.max_per_img)
                ...
                det_bboxes_idxs = torch.cat([seen_det_bboxes_idxs, unseen_det_bboxes_idxs], dim=0)
                # return [seen_det_bboxes, unseen_det_bboxes], [seen_det_labels, unseen_det_labels]
                return det_bboxes, det_labels, det_bboxes_idxs
        ...
        if not self.seen_class:
            semantic_bg = torch.mm(semantic_score, T_bg.t())
            semantic_score = torch.mm(semantic_score, self.vec_unseen)
            semantic_bg = torch.diagonal(semantic_bg, 0)
            semantic_score[:,0] = semantic_bg
            scores = semantic_score
            # wo w2v
            # scores =torch.bmm(semantic_score.unsqueeze(1), atten_bg[:,50:,:].permute(0,2,1)).squeeze(1)

            
            # new_vec = torch.cat((atten_bg.unsqueeze(-2), self.vec.t()[1:].expand(scores.shape[0], -1, -1)),1)
            # scores = torch.bmm(scores.unsqueeze(-2), new_vec).squeeze(-2)
            
            # scores_bg = torch.mm(scores, atten_bg.t())
            # scores_bg = torch.diagonal(scores_bg, 0)
        ...
        if cfg is None:
            ...
            det_bboxes, det_labels, det_bboxes_idxs = multiclass_nms(bboxes, scores,
