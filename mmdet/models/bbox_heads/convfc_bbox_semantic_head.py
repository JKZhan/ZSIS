...
class ConvFCSemanticBBoxHead(BBoxSemanticHead):
    ...
    def __init__(self,
        ...
        #RBT layer 
        self.RBT_layer = nn.MultiheadAttention(300,1)
    ...
    def forward(self, x, res_feats=None, context_feats=None, return_feats=False, resturn_center_feats=False, bg_vector=None):
        ...
        if self.with_semantic:
            ...
            if self.voc is not None:
                ...
                if self.semantic_norm:
                    ...
                    if self.sync_Vb:
                        with torch.no_grad(): 
                            self.vec_all[:, 0] = bg_vector    
                        batch = semantic_score.shape[0]
                        concated_semantic = torch.cat((semantic_score.unsqueeze(1), self.vec_all.t()[0:self.num_classes,:].unsqueeze(0).expand(batch, -1, -1)), 1)
                    else:
                        batch = semantic_score.shape[0]
                        concated_semantic = torch.cat((semantic_score.unsqueeze(1), self.vec_all.t()[1:self.num_classes,:].unsqueeze(0).expand(batch, -1, -1)), 1)
                    #change to (target length, batch size, embed dim)
                    concated_semantic = concated_semantic.permute(1,0,2)
                    RBT_out, _= self.RBT_layer(concated_semantic, concated_semantic, concated_semantic)
                    #change to (batch size, target length, embed dim)
                    RBT_out = RBT_out.permute(1,0,2)
                ...
                if self.sync_T_bg:
                    if self.bbox_train:
                        semantic_bg_score = torch.mm(semantic_score, T_bg.t())
                        semantic_bg_score = torch.diagonal(semantic_bg_score, 0)
                        semantic_score = torch.mm(semantic_score, self.vec)
                        semantic_score[:,0] = semantic_bg_score
                    else:
                         pass
        ...
        if self.with_reg and not self.reg_with_semantic:
            ...
            bbox_pred = None
        if self.with_decoder:
            return semantic_score, bbox_pred, x_semantic, d_semantic_feature, T_bg
        else:
