...
class SemanticFCNMaskHead(nn.Module):
    ...
    def __init__(self,
        ...
        vec_all = torch.tensor(vec_load, dtype=torch.float32)
    ...
    @auto_fp16()
    def forward(self, x, bg_vector=None, det_bboxes_idxs=None,  T_bg=None):
        ...
        #T_bg score
        if self.mask_train:
            mask_T_bg_scores = torch.bmm(T_bg[0:B,:].unsqueeze(1), x.view(B,300,-1))
            mask_T_bg_scores = mask_T_bg_scores.view(B,28,28)
        else:
            test_T_bg = T_bg[det_bboxes_idxs,:]
            mask_T_bg_scores = torch.bmm(test_T_bg.unsqueeze(1), x.view(B,300,-1))
            mask_T_bg_scores = mask_T_bg_scores.view(B,28,28)
       
        # classification module
        mask_pred_seen = self.conv_vec(x)
        mask_pred_seen[:,0,:,:] = mask_T_bg_scores
        ...
        if not self.seen_class and not self.gzsd:
            ...
            # mask_pred = self.con_vec_t(mask_pred_seen)
            mask_pred_unseen = self.conv_vec_unseen(x)
            mask_pred_unseen[:,0,:,:] = mask_T_bg_scores
