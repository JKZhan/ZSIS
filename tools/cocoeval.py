...
class COCOeval:
    ...
    def evaluate(self):
        ...
        # if p.useCats:
        #     p.catIds = np.unique(p.catIds)
        p.maxDets = sorted(p.maxDets)
        self.params = p

        self._prepare()
        # loop tcatIdshrough images, area range, max detection number
    ...
    def summarize(self):
        ...
        def _summarizeDets():
            ...
            stats[0] = _summarize(1, iouThr=.4, maxDets=self.params.maxDets[2])
            ...
            #                       areaRng='large',
            #                       maxDets=self.params.maxDets[2])
            # stats[4] = _summarize(0, maxDets=self.params.maxDets[0])
            # stats[5] = _summarize(0, maxDets=self.params.maxDets[1])
            # stats[6] = _summarize(0, maxDets=self.params.maxDets[2])
            stats[6] = _summarize(0, iouThr=.4, maxDets=self.params.maxDets[2])
...
class Params:
    ...
    def setDetParams(self):
        ...
        self.iouThrs = np.linspace(.4,
                                   0.6,
                                   int(np.round((0.6 - .4) / .1)) + 1,
                                   endpoint=True)
